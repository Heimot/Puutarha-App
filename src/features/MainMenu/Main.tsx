...
const Main = () => {
    ...
    const updateOrder = async (id: string) => {
        let userId = localStorage.getItem('userId');
        let url = process.env.REACT_APP_API_URL;
        let newData = await FetchData({ urlHost: url, urlPath: '/orders/get_order_with_id', urlMethod: 'GET', urlHeaders: 'Auth', urlQuery: `?currentUserId=${userId}&_id=${editData?._id}&date=${chosenDate}` });
        let data;
    ...
    return (
        ...
        <Box sx={{ flexGrow: 1, padding: 3, paddingTop: 9, backgroundColor: '#212121', height: '100%', minHeight: '100vh' }}>
            ...
            <EditingMenu isOpen={isOpen} setIsOpen={(value) => setIsOpen(value)} editData={editData} setEditData={(value) => setEditData(value)} dataSaved={(id: string) => updateOrder(id)} />
        </Box>
    )
}
